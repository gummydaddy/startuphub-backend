...
class FounderProfileViewSet(viewsets.ModelViewSet):
    ...
    def search(self, request):
        """Search founders by name or username"""
        ...
        founders = FounderProfile.objects.filter(
            Q(name__icontains=query) | Q(user__username__icontains=query)
        ).exclude(user=request.user)[:20]  # Limit to 20 results

        serializer = self.get_serializer(founders, many=True)
        return Response(serializer.data)
        ...
        for conn in queryset:
            connections_data.append({
                ...
                'from_founder_details': {
                    ...
                    'profile_image': request.build_absolute_uri(conn.from_founder.profile_image.url) if conn.from_founder.profile_image else None,
                },
                'to_founder_details': {
                    ...
                    'profile_image': request.build_absolute_uri(conn.to_founder.profile_image.url) if conn.to_founder.profile_image else None,
                },
                'status': conn.status,
